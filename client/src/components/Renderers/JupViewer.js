...
class JupViewer extends React.Component {
	...
	parseMD(source) {
		...
		for (var code in source) {
			...
			if (!!old_source && !this.validURL(old_source[1])) {  // if MD line contains src="...", replace src baseURL with that of .ipynb url
  ...
  ReactMarkdownWithEquations(props) { // https://github.com/rexxars/react-markdown/issues/10
      const newProps = {
          ...props,
          plugins: [
              RemarkMathPlugin,
				...
				<Spin spinning={this.state.loading} >
                  ...
                  { (this.props.title) ? 
										<Typography.Title strong style={{
											...
											wordWrap: 'break-word',
											width: '100%',
										}}>{this.props.title}</Typography.Title> : null }
                    { (this.props.subtitle) ? 
										<Typography.Title
											level={4} style={{
												color: this.state.background_text_theme,
											...
											}}>{this.props.subtitle}</Typography.Title> : null }
									</Col>
									<Col span={1}></Col>
								</Row>
